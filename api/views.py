...
def launches_by_rocket(request):
    ...
    if not rocket_id:
        return JsonResponse({'error': f'Rocket named {rocket_name} not found'}, status=404)
    ...
    filtered_launches = [launch for launch in launches if launch.get('rocket') == rocket_id]
